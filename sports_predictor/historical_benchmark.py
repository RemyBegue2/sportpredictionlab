    ...
    for _, event in event_model.iterrows():
        ...
        for key in event_keys:
            ...
            rows.append({
                ...
                "blend50_away": 0.5 * float(event["model_away"]) + 0.5 * float(consensus["probabilities"][away]),
                "blend50_draw": 0.5 * float(event["model_draw"]) + 0.5 * float(consensus["probabilities"]["Draw"]),
                "blend50_home": 0.5 * float(event["model_home"]) + 0.5 * float(consensus["probabilities"][home]),
